...
const configureCookieConsent = () => {
  ...
  const categories = () => {
    const configuredCategories = config.categories ? config.categories.split(',').map((n) => n.toLowerCase().trim()) : ['essential', 'analytics'];
    return {
      [CC.UNCATEGORIZED]: configuredCategories.includes('uncategorized'),
      [CC.ESSENTIAL]: configuredCategories.includes('essential'),
      [CC.PERSONALIZATION]: configuredCategories.includes('personalization'),
      [CC.ANALYTICS]: configuredCategories.includes('analytics'),
      [CC.MARKETING]: configuredCategories.includes('marketing'),
    }
  };

  const cc = new CC({
    // options: https://github.com/sandstreamdev/cookieconsent/blob/dev/src/options/popup.js#L25
    type: config.type || 'opt-in',
    container: document.querySelector('footer'),
    consentSettingsElementId: REVOKE_CONSENT_ELEMENT_ID,
    layout: config.layout || 'categories',
    cookie: {
...
*/
  function applyConsent() {
    const { consents } = cc;
    Object.entries(consents).forEach(([k, v]) => sampleRUM('consent', { source: k, target: v }));
  }
...
export default function loadCookieConsent(_config) {
  config = _config;
